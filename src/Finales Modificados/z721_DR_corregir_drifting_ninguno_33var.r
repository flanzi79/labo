...
AgregarVariables_IntraMes  <- function( dataset )
{
  ...
  #Proporción de de ingresos destinada a inversión a largo plazo, quien invierte a largo plazo generalmente no saldría del banco
  dataset[  , proporcion_inv_largo_plazo  := (((minversion1_pesos+minversion1_dolares+minversion2)/cliente_antiguedad))/(mpayroll+mpayroll2) ]
  
  #Meses promedio de cancelación de prestamos, una persona con prestamos en el banco no se daría de baja.
  dataset[  , tiempo_promedio_pago_prestamos  := ((mprestamos_personales+mprestamos_prendarios+mprestamos_hipotecarios)/(mpayroll+mpayroll2)) ]
  
  #Relación entre consumo de tarjetas y saldos de cuentas
  dataset[  , consumos_tc_sobre_saldos  := ((Master_mconsumospesos+Master_mconsumosdolares+Visa_mconsumospesos+Visa_mconsumosdolares)/(mcuentas_saldo)) ]

  #Variables generadas en experimentos colaborativos y etiquetadas como las mejores obtenidas
